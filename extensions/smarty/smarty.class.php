...
class Smarty extends Smarty_Internal_TemplateBase {
	...
	/**
	 * error handler returned by set_error_hanlder() in
	 * Smarty::muteExpectedErrors()
	 */
	public static $_previous_error_handler = null;
	/**
	 * contains directories outside of SMARTY_DIR that are to be muted by
	 * muteExpectedErrors()
	 */
	 ...
	 *
	 * @param $security_class string|Smarty_Security
	 ...
	 *
	 * @param $template_dir string|array
	 ...
	 *
	 * @param $plugins_dir string|array
	 ...
	 * @param $type string
	 *       	 "pre", "output", … specify the filter type to set. Defaults
	 *        	to none treating $filters' keys as the appropriate types
	 * @return Smarty current Smarty instance for chaining
	 */
	 ...
	 *
	 * @param $type string
	 *       	 type of filter to get autoloads for. Defaults to all autoload
	 *        	filters
	 * @return array array( 'type1' => array( 'filter1', 'filter2', … ) ) or
	 *         array( 'filter1', 'filter2', …) if $type was specified
	 */
	...
	public static function mutingErrorHandler($errno, $errstr, $errfile, $errline, $errcontext) {
		...
		// pass to next error handler if this error did not occur inside
		// SMARTY_DIR
	...
	public static function muteExpectedErrors() {
		 ...
		 * reason did not understand the following paragraph: It is important to
		 * remember that the standard PHP error handler is completely bypassed
		 * for the error types specified by error_types unless the callback
		 * function returns FALSE. error_reporting() settings will have no
		 * effect and your error handler will be called regardless - however you
		 ...
		 * operator. Smarty deliberately uses @filemtime() over file_exists()
		 * and filemtime() in some places. Reasons include - @filemtime() is
		 * almost twice as fast as using an additional file_exists() - between
		 * file_exists() and filemtime() a possible race condition is opened,
