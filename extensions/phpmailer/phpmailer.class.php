 ...
 * ~ class.phpmailer.php
 * .---------------------------------------------------------------------------.
 * | Software: PHPMailer - PHP email class | | Version: 5.1 | | Contact: via
 * sourceforge.net support pages (also www.worxware.com) | | Info:
 * http://phpmailer.sourceforge.net | | Support:
 * http://sourceforge.net/projects/phpmailer/ | |
 * ------------------------------------------------------------------------- | |
 * Admin: Andy Prevost (project admininistrator) | | Authors: Andy Prevost
 * (codeworxtech) codeworxtech@users.sourceforge.net | | : Marcus Bointon
 * (coolbru) coolbru@users.sourceforge.net | | Founder: Brent R. Matzelle
 * (original founder) | | Copyright (c) 2004-2009, Andy Prevost. All Rights
 * Reserved. | | Copyright (c) 2001-2003, Brent R. Matzelle | |
 * ------------------------------------------------------------------------- | |
 * License: Distributed under the Lesser General Public License (LGPL) | |
 * http://www.gnu.org/copyleft/lesser.html | | This program is distributed in
 * the hope that it will be useful - WITHOUT | | ANY WARRANTY; without even the
 * implied warranty of MERCHANTABILITY or | | FITNESS FOR A PARTICULAR PURPOSE.
 * | | -------------------------------------------------------------------------
 * | | We offer a number of paid services (www.worxware.com): | | - Web Hosting
 * on highly optimized fast and secure servers | | - Technology Consulting | | -
...
class PHPMailer {
	...
	function SetLanguage($langcode = 'en', $lang_path = 'language/') {
		...
		// Overwrite language-specific strings. This way we'll never have
		// missing translations - no more "language string failed to load"!
		$l = true;
		...
		while ( ! $foundSplitPos ) {
			...
			if ($encodedCharPos !== false) {
				...
				if ($dec < 128) { // Single byte character.
				                  // If the encoded char was found at pos 0, it will
				                  // fit
				                  // otherwise reduce maxLength to start of the
				                  // encoded char
					...
					$foundSplitPos = true;
				} elseif ($dec < 192) { // Middle byte of a multi byte character,
				                        // look further back
					$lookBack += 3;
				}
			} else {
	...
	public function CreateBody() {
		...
		switch ($this->message_type) {
			...
			case 'alt_attachments' :
				...
				$body .= $this->GetBoundary ( $this->boundary [2], '', 'text/plain', '' ) . $this->LE; // Create
				                                                                                   // text
				                                                                                   // body
				$body .= $this->EncodeString ( $this->AltBody, $this->Encoding );
				$body .= $this->LE . $this->LE;
				$body .= $this->GetBoundary ( $this->boundary [2], '', 'text/html', '' ) . $this->LE; // Create
				                                                                                  // the
				                                                                                  // HTML
				                                                                                  // body
				$body .= $this->EncodeString ( $this->Body, $this->Encoding );
	 ...
	 * @param $encoding string
	 *       	 The encoding to use; one of 'base64', '7bit', '8bit',
	...
	public function HasMultiBytes($str) {
		...
		} else { // Assume no multibytes (we can't handle without mbstring
		         // functions anyway)
			return false;
		}
	}
	...
	public function EncodeQPphp($input = '', $line_max = 76, $space_conv = false) {
		...
		while ( list ( , $line ) = each ( $lines ) ) {
			...
			for($i = 0; $i < $linlen; $i ++) {
				$c = substr ( $line, $i, 1 );
				$dec = ord ( $c );
				if (($i == 0) && ($dec == 46)) { // convert first point in the
				                                       // line into =2E
					$c = '=2E';
				}
				if ($dec == 32) {
				...
				} elseif (($dec == 61) || ($dec < 32) || ($dec > 126)) { // always
				                                                            // encode "\t",
				                                                            // which is
				                                                            // *not*
				                                                            // required
				...
				if ((strlen ( $newline ) + strlen ( $c )) >= $line_max) { // CRLF is not
				                                                      // counted
	 ...
	 * Uses a PHP5 stream filter to do the encoding about 64x faster than the
	 * old version
	 ...
	 * @param $line_max integer
	 *       	 Number of chars allowed on a line before wrapping
	 * @param $space_conv boolean
	 *       	 Dummy param for compatibility with existing EncodeQP function
	 * @return string
	 * @author Marcus Bointon
	 */
	public function EncodeQP($string, $line_max = 76, $space_conv = false) {
		if (function_exists ( 'quoted_printable_encode' )) { // Use native function
		                                                  // if it's available (>=
		                                                  // PHP5.3)
		...
		if (! in_array ( 'convert.*', $filters )) { // Got convert stream filter?
			return $this->EncodeQPphp ( $string, $line_max, $space_conv ); // Fall
			                                                            // back to old
			                                                            // implementation
		}
		$fp = fopen ( 'php://temp/', 'r+' );
		$string = preg_replace ( '/\r\n?/', $this->LE, $string ); // Normalise line
		                                                       // breaks
		...
		$out = preg_replace ( '/^\./m', '=2E', $out ); // Encode . if it is first
		                                            // char on a line, workaround for
		                                            // bug in Exchange
		fclose ( $fp );
		return $out;
	}
	...
	public function EncodeQ($str, $position = 'text') {
		...
		switch (strtolower ( $position )) {
			...
			default :
				...
				// TODO using /e (equivalent to eval()) is probably not a good
				// idea
	...
	/**
	 * Evaluates the message and returns modifications for inline images and
	 * backgrounds
	 * 
	 * @access public
	 * @return $message
	 ...
	 * @param $value mixed
	 *       	 Parameter Value
	 *       	 NOTE: will not work with arrays, there are no arrays to
	 *        	set/reset
	 ...
	 *
	 * @access public
	 * @param $key_filename string
	 *       	 Parameter File Name
	 * @param $key_pass string
	...
	public function DKIM_HeaderC($s) {
		...
		foreach ( $lines as $key => $line ) {
			...
			$lines [$key] = $heading . ":" . trim ( $value ); // Don't forget to remove WSP
			                                         // around the value
		}
		$s = implode ( "\r\n", $lines );
		return $s;
	...
	public function DKIM_Add($headers_line, $subject, $body) {
		...
		$DKIMtime = time (); // Signature Timestamp = seconds since
		                                 // 00:00:00 - Jan 1, 1970 (UTC time zone)
		...
		$subject = str_replace ( '|', '=7C', $this->DKIM_QP ( $subject_header ) ); // Copied
		                                                                     // header
		                                                                     // fields
