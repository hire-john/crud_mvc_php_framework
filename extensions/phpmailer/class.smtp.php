 ...
 * sourceforge.net support pages (also www.codeworxtech.com) | | Info:
 ...
 * | | -------------------------------------------------------------------------
 * | | We offer a number of paid services (www.codeworxtech.com): | | - Web
 * Hosting on highly optimized fast and secure servers | | - Technology
 * Consulting | | - Oursourcing (highly qualified programmers and graphic
 * designers) |
 * '---------------------------------------------------------------------------'
 */
 ...
 * @license http://www.gnu.org/copyleft/lesser.html Distributed under the Lesser
...
class SMTP {
	...
	public function Connect($host, $port = 0, $tval = 30) {
		...
		// SMTP server can take longer to respond, give longer timeout for first
		// read
	...
	public function Data($msg_data) {
		...
		/*
		 * the server is ready to accept data! according to rfc 821 we should
		 * not send more than 1000 including the CRLF characters on a single
		 * line so we will break the data up into lines by \r and/or \n then if
		 * needed we will break each of those into smaller lines to fit within
		 * the limit. in addition we will be looking for lines that start with a
		 * period '.' and append and additional period '.' to that line. NOTE:
		 * this does not count towards limit.
		 */
		...
		/*
		 * we need to find a good way to determine is headers are in the
		 * msg_data or if it is a straight msg body currently I am assuming rfc
		 * 822 definitions of msg headers and if the first field of the first
		 * line (':' sperated) does not contain a space then it _should_ be a
		 * header and we can process all lines before a blank "" line as
		 * headers.
		 */
		...
		while ( list ( , $line ) = @each ( $lines ) ) {
			...
			while ( strlen ( $line ) > $max_line_length ) {
				...
				/*
				 * if processing headers add a LWSP-char to the front of new
				 * line rfc 822 on long msg headers
				 */
				if ($in_headers) {
					$line = "\t" . $line;
				}
